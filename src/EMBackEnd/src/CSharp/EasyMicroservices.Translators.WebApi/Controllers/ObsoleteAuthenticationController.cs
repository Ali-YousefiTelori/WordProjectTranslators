...
//    public class ObsoleteAuthenticationController : ControllerBase
//    {
//        AppUnitOfWork _appUnitOfWork;
//        public ObsoleteAuthenticationController(AppUnitOfWork appUnitOfWork)
//        {
...
//            var getOrCreateUser = await new LogicBase<UserEntity, UserEntity>(context).Find(query => query.Where(x => x.UserName == userName));
//            if (!getOrCreateUser.IsSuccess)
//                return getOrCreateUser.ToContract<UserContract>();
//            else if (getOrCreateUser.Result != null)
//            {
//                return (FailedReasonType.Duplicate, "نام کاربری تکراری است!");
//            }
//            var key = Guid.NewGuid();
//            var userResult = await new LogicBase<UserEntity>(context).Add(new UserEntity()
//            {
//                IsConfirmed = false,
//                UserName = userName,
//                UserSession = key,
...
//        {
//            using var context = _appUnitOfWork.GetContext();
//            var getOrCreateUser = await new LogicBase<UserEntity, UserEntity>(context).Find(query => query.Include(x => x.UserPermissions).Where(x => x.UserSession == session));
//            if (!getOrCreateUser.IsSuccess)
//                return getOrCreateUser.ToContract<UserContract>();
//            else if (getOrCreateUser.Result != null)
//            {
...
//                return user;
//            }
//            return (FailedReasonType.Incorrect, "اطلاعات شما جهت ورود اشتباه وارد شده است.");
//        }
